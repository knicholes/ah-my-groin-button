...
#define DEBUG  // Enable debug output for testing and troubleshooting
...
#include <Arduino.h>
 ...
 // Device settings
 const int DEFAULT_VOLUME = 30;      // Volume level (0-30)
 const int NUM_AUDIO_FILES = 3;     // Number of audio files (0001.wav to 0010.wav)
 const int MIN_TRACK_NUMBER = 1;     // First track number (0001.wav)
 const int MAX_TRACK_NUMBER = 3;    // Last track number (0010.wav)
 ...
 void readDFPlayerResponse();  // Ensure forward declaration is here
 ...
 void setup() {
   ...
   if (waitForDFPlayerReady()) {
     ...
     // Reapplying: Query file count in /mp3 folder (command 0x47, parameter 1 for /mp3)
     sendDFPlayerCommand(0x47, 1);
     readDFPlayerResponse();  // Print response with file count
   } else {
     #ifdef DEBUG
   ...
   delay(100);
   enterSleepMode();
   delay(500);  // Delay to stabilize before attaching interrupt
   attachInterrupt(digitalPinToInterrupt(BUTTON_PIN), buttonInterrupt, FALLING);  // Attach after setup
 }

 void handleButtonPress() {
  delay(500);  // Added longer delay to debounce noise after wake
 ...
 void playRandomSound() {
   ...
   if (waitForDFPlayerReady()) {
     ...
     // Query playback status (command 0x42)
     sendDFPlayerCommand(0x42, 0);
     readDFPlayerResponse();
 ...
 bool waitForDFPlayerReady() {
   unsigned long startTime = millis();
   while (millis() - startTime < 5000) {  // Extended timeout
     ...
     #endif
     delay(500);  // Check every 500ms with print
   }
   #ifdef DEBUG
   Serial.println(F("Timeout: No response from DFPlayer"));
   #endif
   return false;
 }
...
// New function to read DFPlayer responses for debugging
void readDFPlayerResponse() {
  delay(100);  // Short delay to allow response
  #ifdef DEBUG
  if (dfPlayerSerial.available()) {
    Serial.println(F("DFPlayer response:"));
    while (dfPlayerSerial.available()) {
      uint8_t byte = dfPlayerSerial.read();
      Serial.print(F("0x"));
      Serial.println(byte, HEX);
    }
  } else {
    Serial.println(F("No response from DFPlayer"));
  }
  #endif
}
 ...
 void buttonInterrupt() {
   ...
   buttonPressed = true;
   #ifdef DEBUG
   Serial.println(F("Interrupt triggered"));  // Added for diagnostics
   #endif
 }
